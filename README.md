...
| ![ice patch](docs/images/obstacles/ice-patch.svg) | `ice-patch` | `standard` | Static patch obstacle (no lane switching). | Applies speed-boost effect for `1400ms`; no life loss from patch contact. | Not jump-clear relevant. | Patch persists after contact; repeated patch hits stack boost intensity/duration (with caps). |
