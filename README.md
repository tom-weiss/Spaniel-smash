...
`v1.2.0` adds a steeper late-game difficulty curve with extra obstacle burst spawns, faster boss poo-bag pressure, and longer ski-school snakes at high levels. It also adds a level-10 victory finale with fireworks + celebration music.
...
- `ski-school-snake` joins rare spawns at level 3; it starts with 3 children and adds 3 children per level after 3 (up to 24).
- Obstacle spawn cadence tightens harder every level (with a lower floor) and can add bonus extra spawns on higher levels, so total obstacle pressure ramps sharply.
...
- Andy throws `poo-bag` projectiles much faster on later levels via steeper level-scaled throw cooldowns; high levels can launch multiple bags at once across nearby lanes.
- Level-up now includes a pulse + sparkle celebration animation behind the banner.
- Defeating Andy on level 10 triggers a victory finale screen: "Congratulations you beat Evil Andy" with fireworks and celebration music.
...
Tier cadence reference: baseline spawn starts at about `520ms` on level 1, tightens by roughly `70ms` per level down to about `90ms`, and uses a faster post-level transition burst (about `150ms` floor). `rare` remains about every `10-20s`, `super-rare` about every `60-600s`, and `mythic` about every `30-90s` after mythic unlock.
